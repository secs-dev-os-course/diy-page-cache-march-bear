...
struct cache_list cache = {.capacity = 256, .length = 0, .first = NULL, .last = NULL};
...
int cache_node_fsync(struct cache_node* node) {
  ...
  ssize_t res =
      pwrite(node->key.fd, node->data + node->ioff, node->size, node->key.offset + node->ioff);
...
int cache_put_buf(
  ...
  while (count != 0) {
    ...
    struct cache_node* removed =
        cache_put(cache, (page_key){fd, offset}, to_wr, curr_ioff, curr_buf, curr_count);
...
size_t cache_get_buf(struct cache_list* cache, int fd, off_t offset, void* data_buf, size_t count) {
